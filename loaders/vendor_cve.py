...
def parse_vendor_cve_records(rec: dict) -> dict:
    return {
        ...
        "patched_date": rec.get("patched_date", None),
    }

def upsert_vendor_cve(conn, rec: dict) -> None:
    """Inserting the records also checking for conflicts simultaneously."""
    parsed = parse_vendor_cve_records(rec)
    vendor_id = get_or_create_vendor_id(conn, parsed["vendor_name"])
    bulletin_ids = get_bulletin_ids(conn, vendor_id, parsed["bulletin_title"], parsed["published_date"])
    bulletin_id = bulletin_ids[0] if bulletin_ids else None
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            """,
            (vendor_id, parsed["cve_id"], bulletin_id, parsed["patched_date"]),
        )
